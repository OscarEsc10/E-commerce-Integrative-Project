...
class EbooksDashboard {
  ...
  toggleFeaturesByRole() {
    ...
    ['add-ebook-btn','btn-cart'].forEach(hide);

    if (role === 'admin') show('add-ebook-btn');
    if (role === 'seller') {
      show('add-ebook-btn');
      show('btn-cart');
    }
    if (role === 'customer') show('btn-cart');
  }
  ...
  async populateProfile() {
    ...
    try {
      if (typeof apiClient.getProfile === 'function') {
        ...
        user = resp?.data?.user || resp?.user || user;
        this.user = user;
        authManager.setUserData(user);
      }
    } catch (err) {
      console.warn('No se pudo refrescar perfil, usando datos locales:', err);
    }

    profileContent.innerHTML = `
      <div class="bg-gray-50 p-4 rounded-lg">
        <label class="block font-semibold text-gray-700 mb-2">Nombre Completo:</label>
        <span class="text-gray-600">${user.full_name || user.name || '—'}</span>
      </div>
      <div class="bg-gray-50 p-4 rounded-lg">
      ...
      <div class="bg-gray-50 p-4 rounded-lg">
        ...
        <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase ${this.getRoleBadgeClass(user)}">${user.role || '—'}</span>
      </div>
      <div class="bg-gray-50 p-4 rounded-lg">
  ...
  async loadCategories() {
    try {
      ...
      this.categories = Array.isArray(resp) ? resp : resp.data || [];
  ...
  populateCategorySelect() {
    const select = document.getElementById('ebook-category');
    if (!select) return;
